...
public class LootDropEffect implements Runnable {
	...
	public LootDropEffect(Location location, String lootTable) {
		...
		this.taskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(MCFCore.getInstance(), this, 5L, 5L);
		...
		Location beaconLocation = new Location(location.getWorld(), location.getX(), location.getY() - 1, location.getZ());
	...
	public void undoBlocks() {
		...
		MCFCore.getInstance().getLootDropManager().spawnChest(location, lootTable);
